    ...
    stage('Publish') {
      ...
      steps {
        withPublishEnvironment {
          ...
          sh "go run pkg/build/upload/github/github.go dist/scm-cli.json scoop-bucket main bucket/scm-cli.json \"Update scoop scm-cli to ${releaseVersion}\""
          sh "go run pkg/build/upload/scmm/scmm.go dist/scm-cli.rb homebrew-tap master Formula/scm-cli.rb \"Update brew scm-cli to ${releaseVersion}\""
          sh "go run pkg/build/descriptor/app.go dist > dist/release.yaml"
          sh "go run pkg/build/upload/scmm/scmm.go dist/release.yaml website master content/cli/releases/${hyphenatedReleaseVersion}.yaml \"Release cli version ${releaseVersion}\""
        }
      }
    }
...
void withPublishEnvironment(Closure<Void> closure) {
  withCredentials([
    usernamePassword(credentialsId: 'packages-scm-manager-org', usernameVariable: 'UPLOAD_DEFAULT_USERNAME', passwordVariable: 'UPLOAD_DEFAULT_SECRET'),
    usernamePassword(credentialsId: 'packages-scm-manager-org', usernameVariable: 'UPLOAD_RPM_USERNAME', passwordVariable: 'UPLOAD_RPM_SECRET'),
    usernamePassword(credentialsId: 'packages-scm-manager-org', usernameVariable: 'UPLOAD_DEB_USERNAME', passwordVariable: 'UPLOAD_DEB_SECRET'),
    file(credentialsId: 'gpg_packages-scm-manager-org', variable: 'GPG_KEY_PATH'),
